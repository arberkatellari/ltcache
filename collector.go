...
func ReadAll(fldrPath string, cfg map[string]*CacheConfig) (tc *TransCache, err error) {
	...
	for i := range fileNames {
		...
		go func() {
			...
			var cacheColl map[string]*CacheCollector
			// for {
			todmpTime1 = time.Now()
			...
			mapMu.Lock()
			todmpTime1 = time.Now()
			tc.cache[fileNames[i]] = newCacheFromDump(cacheColl, cfg[fileNames[i]].MaxItems, cfg[fileNames[i]].TTL, cfg[fileNames[i]].StaticTTL, cfg[fileNames[i]].OnEvicted)
...
func (coll *InstanceCollector) WriteAll() error {
	...
	var once sync.Once
	for cachingInstance, CacheColl := range coll.SetColl {
		wg.Add(1)
		go func() {
			defer wg.Done()
			...
			todmpTime1 = time.Now()
			if err := enc.Encode(CacheColl); err != nil {
				once.Do(func() {
					errChan <- fmt.Errorf("failed to encode CacheColl for %s: %w", cachingInstance, err)
					// return ffmt.Errorf("failed to encode CacheColl for %s: %w", cachingInstance, err)
				})
				return
			}
