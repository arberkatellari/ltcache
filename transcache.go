...
func NewTransCacheWithOfflineCollector(fldrPath string, dumpInterval, rewriteInterval time.Duration, writeLimit int, cfg map[string]*CacheConfig, l logger) (tc *TransCache, err error) {
	...
	for cacheName, config := range tc.cfg { // range over cfg to create each cache and populate TransCache.cache with them
		if err := ensureDir(path.Join(fldrPath, cacheName)); err != nil {
			return nil, err
		}
		wg.Add(1)
		go func() {
			defer wg.Done()
			offColl := NewOfflineCollector(path.Join(fldrPath, cacheName), writeLimit,
				(dumpInterval != -1), l, dumpInterval, rewriteInterval)
...
func (tc *TransCache) Shutdown() error {
	for _, c := range tc.cache {
		if c.offCollector == nil {
			return nil // dont return any error on shutdown where collector was disabled
		}
...
func (tc *TransCache) BackupDumpFolder(backupFolderPath string, zip bool) error {
	var dmpCacheFldrPath string      // path to the Cache Dump folder
	for _, cache := range tc.cache { // lock all dumping or rewriting until function returns
		...
		dmpCacheFldrPath = cache.offCollector.fldrPath // get the path to cache dump, parent path will be extracted after
	...
	// parentDmpFlrd will be the parent folder, i.e.: "/tmp/internal_db/datadb" out of "/tmp/internal_db/datadb/*default"
	parentDmpFlrd := filepath.Dir(dmpCacheFldrPath)
	cdFldr := filepath.Dir(parentDmpFlrd)                      // current directory to go to, so the zipped file doesnt unnecesary take the parent folder path "/tmp/internal_db"
	fldrToZip := strings.TrimPrefix(parentDmpFlrd, cdFldr+"/") // name of folder that will be zipped "/datadb"
	// runs the 'zip -r' command
	if zip {
		// runs sh -c "cd /tmp/internal_db/ && zip -r /backupfoldr/Backup_123 datadb"
		return runCommand("sh", []string{"-c", "cd " + cdFldr + " && zip -r " +
			path.Join(backupFolderPath, "Backup_"+strconv.FormatInt(time.Now().UnixMilli(),
				10)+".zip") + " " + fldrToZip})
	}
	// runs the 'cp -r' command
	return runCommand("cp", []string{"-r", parentDmpFlrd, path.Join(backupFolderPath,
